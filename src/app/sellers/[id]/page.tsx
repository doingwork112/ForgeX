      ...
      <main className="mx-auto max-w-6xl px-6 py-14">
        ...
        <div className="mb-12 flex flex-col gap-6 sm:flex-row sm:items-start">
          ...
          <div className="flex-1">
            ...
            <button className="rounded-xl border border-black/[0.08] px-5 py-2.5 text-sm font-medium text-muted-foreground hover:text-[#111] hover:border-black/[0.15] transition-colors">
              Contact
            </button>
          </div>
        </div>
        ...
        <div>
          ...
          {sellerApps.length === 0 ? (
            <div className="flex h-48 flex-col items-center justify-center gap-3 rounded-2xl border border-black/[0.06] bg-white text-muted-foreground">
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {sellerApps.map((app) => (
                <Link key={app.id} href={`/marketplace/${app.id}`}>
                  <div className="group rounded-2xl border border-black/[0.06] bg-white overflow-hidden transition-all duration-300 hover:border-[#1D9E75]/40 hover:shadow-[0_0_28px_rgba(29,158,117,0.12)] hover:-translate-y-0.5 cursor-pointer">
                    ...
                    <div className="p-5">
                      <div className="mb-1 flex items-start justify-between gap-2">
                        <h3 className="font-semibold text-[#111]">{app.name}</h3>
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex flex-wrap gap-1">
                          {app.techStack.slice(0, 2).map((tech) => (
                            <span
                              key={tech}
                              className="rounded-md border border-black/[0.08] bg-white px-2 py-0.5 text-[11px] text-muted-foreground"
                            >
                              {tech}
                            </span>
                          ))}
