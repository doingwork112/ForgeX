...
export default function UserProfilePage({ params }: { params: { username: string } }) {
  ...
  // Tab data
  const [activeTab, setActiveTab] = useState<"apps" | "bounties" | "sales">("apps");
  const [myApps, setMyApps] = useState<App[]>([]);
  const [myBounties, setMyBounties] = useState<Bounty[]>([]);
  const [mySales, setMySales] = useState<(Order & { app_name?: string; buyer_name?: string })[]>([]);
  ...
  /* ---------------------------------------------------------------- */
  /*  Fetch tab data (apps, bounties, sales)                           */
  /* ---------------------------------------------------------------- */

  async function fetchTabData(userId: string) {
    setTabLoading(true);

    const [appsRes, bountiesRes, salesRes] = await Promise.all([
    ...
    if (orders.length > 0) {
      ...
      setMySales(
        orders.map((o) => ({ ...o, app_name: appMap.get(o.app_id) ?? "Unknown App" }))
      );
    } else {
      setMySales([]);
    }
  ...
  /* ---------------------------------------------------------------- */
  /*  Upload helpers — instant local preview + background upload       */
  /* ---------------------------------------------------------------- */
      ...
      <main>
        ...
        <div className="mx-auto max-w-4xl px-4 sm:px-6">
          ...
          <div className="relative -mt-12 mb-6 flex items-end justify-between">
            ...
            <div className="flex items-center gap-5 pb-1">
              <div className="text-center">
                <p className="text-sm font-bold text-[#111]">{myApps.length}</p>
                <p className="text-[11px] text-gray-500">Apps</p>
              </div>
              <div className="text-center">
                <p className="text-sm font-bold text-[#111]">{totalSales}</p>
                <p className="text-[11px] text-gray-500">Sales</p>
              </div>
              <div className="text-center">
                <p className="text-sm font-bold text-[#111]">{myBounties.length}</p>
                <p className="text-[11px] text-gray-500">Bounties</p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-8 space-y-3">
            ...
            <div>
              {isOwner && editingName ? (
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") { setEditingName(false); saveField({ display_name: displayName || null }); }
                  }}
                ...
                <div className="flex items-center gap-2 group/name">
                  ...
                  {isOwner && (
                    <button onClick={() => setEditingName(true)} className="opacity-0 group-hover/name:opacity-100 transition-opacity p-1 rounded-lg hover:bg-black/[0.04]" title="Edit name">
                      <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} className="text-gray-400"><path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                    </button>
                  )}
                </div>
              )}
              ...
              {isOwner && editingBio ? (
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => { setEditingBio(false); saveField({ bio: bio || null }); }} className="rounded-lg bg-[#1D9E75] px-3 py-1.5 text-xs font-medium text-white hover:bg-[#178c66] transition-colors">Save</button>
                    <button onClick={() => { setEditingBio(false); setBio(profile?.bio ?? ""); }} className="rounded-lg px-3 py-1.5 text-xs font-medium text-gray-500 hover:bg-black/[0.04] transition-colors">Cancel</button>
                ...
                <div className="flex items-start gap-2">
                  {bio ? (
                    ...
                    <button onClick={() => setEditingBio(true)} className="text-sm text-gray-400 italic hover:text-[#1D9E75]">Click to add a bio...</button>
                  ) : null}
                  {isOwner && bio && (
                    <button onClick={() => setEditingBio(true)} className="opacity-0 group-hover/bio:opacity-100 transition-opacity p-1 rounded-lg hover:bg-black/[0.04] shrink-0 mt-0.5" title="Edit bio">
                      <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} className="text-gray-400"><path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                    </button>
                  )}
                </div>
            ...
            <div className="group/status">
              {isOwner && editingStatus ? (
                <div className="flex items-center gap-2">
                  <input type="text" value={currentlyBuilding} onChange={(e) => setCurrentlyBuilding(e.target.value)} autoFocus placeholder="What are you working on?" className="rounded-lg border border-[#1D9E75]/30 bg-white px-3 py-1.5 text-xs text-[#111] placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#1D9E75]/10 w-64"
                    onKeyDown={(e) => { if (e.key === "Enter") { setEditingStatus(false); saveField({ currently_building: currentlyBuilding || null }); } }}
                  />
                  <button onClick={() => { setEditingStatus(false); saveField({ currently_building: currentlyBuilding || null }); }} className="rounded-lg bg-[#1D9E75] px-3 py-1.5 text-xs font-medium text-white hover:bg-[#178c66] transition-colors">Save</button>
                  <button onClick={() => { setEditingStatus(false); setCurrentlyBuilding(profile?.currently_building ?? ""); }} className="rounded-lg px-3 py-1.5 text-xs font-medium text-gray-500 hover:bg-black/[0.04] transition-colors">Cancel</button>
                </div>
              ) : (
                <div className="flex items-center gap-2">
                  {currentlyBuilding ? (
                    ...
                    <button onClick={() => setEditingStatus(true)} className="text-xs text-gray-400 italic hover:text-[#1D9E75]">+ Set what you&apos;re building</button>
                  ) : null}
                  {isOwner && currentlyBuilding && (
                    <button onClick={() => setEditingStatus(true)} className="opacity-0 group-hover/status:opacity-100 transition-opacity p-1 rounded-lg hover:bg-black/[0.04]" title="Edit status">
                      <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} className="text-gray-400"><path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                    </button>
                  )}
                </div>
            ...
            <div className="group/links">
              {isOwner && editingLinks ? (
                <div className="space-y-3 max-w-md">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => { setEditingLinks(false); saveField({ website: website || null, twitter: twitter || null }); }} className="rounded-lg bg-[#1D9E75] px-3 py-1.5 text-xs font-medium text-white hover:bg-[#178c66] transition-colors">Save</button>
                    <button onClick={() => { setEditingLinks(false); setWebsite(profile?.website ?? ""); setTwitter(profile?.twitter ?? ""); }} className="rounded-lg px-3 py-1.5 text-xs font-medium text-gray-500 hover:bg-black/[0.04] transition-colors">Cancel</button>
                  </div>
                </div>
              ) : (
                <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500">
                  ...
                  {isOwner && !website && !twitter && (
                    <button onClick={() => setEditingLinks(true)} className="text-xs text-gray-400 italic hover:text-[#1D9E75]">+ Add links</button>
                  )}
                  {isOwner && (website || twitter) && (
                    <button onClick={() => setEditingLinks(true)} className="opacity-0 group-hover/links:opacity-100 transition-opacity p-1 rounded-lg hover:bg-black/[0.04]" title="Edit links">
                      <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} className="text-gray-400"><path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                    </button>
                  )}
                </div>
          ...
          <div className="flex gap-1 border-b border-black/[0.06] mb-6">
            {([
              { key: "apps" as const, label: "My Apps", count: myApps.length },
              { key: "bounties" as const, label: "Bounties", count: myBounties.length },
              { key: "sales" as const, label: "Sales History", count: mySales.length },
            ]).map((tab) => (
              <button
                key={tab.key}
          ...
          <div className="pb-16">
            {tabLoading ? (
              ...
              <>
                ...
                {activeTab === "apps" && (
                  myApps.length === 0 ? (
                    <EmptyState icon="📦" text="No apps listed yet" />
                  ) : (
                      ...
                      {myApps.map((app) => (
                        <Link key={app.id} href={`/marketplace/${app.id}`}>
                          <div className="group rounded-2xl border border-black/[0.06] bg-white p-5 hover:border-[#1D9E75]/30 hover:shadow-sm transition-all">
                            ...
                            {app.category && (
                              <span className="inline-block mt-2 rounded-full bg-black/[0.03] px-2.5 py-0.5 text-[11px] text-gray-500">{app.category}</span>
                            )}
                          </div>
                        </Link>
                      ))}
                      ...
                      {mySales.map((order) => (
                        <div key={order.id} className="rounded-2xl border border-black/[0.06] bg-white p-5">
                          <div className="flex items-start justify-between">
                            <div>
                              ...
                              <p className="text-xs text-gray-500 mt-0.5">
                                {order.plan === "custom" ? "Custom plan" : "Basic plan"} &middot; Order #{order.id.slice(0, 8)}
                              </p>
                            </div>
                          ...
                          <div className="flex items-center gap-4 mt-3 pt-3 border-t border-black/[0.04] text-xs text-gray-500">
                            ...
                            {order.tail_payment > 0 && <span>Remaining: ${(order.tail_payment / 100).toFixed(2)}</span>}
